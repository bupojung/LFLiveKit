...
@property (nonatomic, strong) GPUImageOutput<GPUImageInput> *myFilter;
...
- (UIButton *)beautyButton {
    if (!_beautyButton) {
        ...
        [_beautyButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id sender) {
            if (_self.session.filter == nil) {
                _self.session.filter = _self.myFilter;
            }else {
                _self.session.filter = nil;
            }
...
- (GPUImageOutput<GPUImageInput> *)myFilter {
    if (_myFilter == nil) {
        UIImageView *imageview = [[UIImageView alloc]initWithFrame:CGRectMake(60, 100, 65, 22)];
        imageview.image = [UIImage imageNamed:@"huzi.png"];
        LFGPUImageBeautyFilter *beauty = [[LFGPUImageBeautyFilter alloc]init];
